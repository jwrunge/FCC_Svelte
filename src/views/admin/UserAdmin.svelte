<script>
  ...
  async function load() {
    ...
    error = "";
    try {
      const resp = await fetch("/data/php/admin_user_list.php?start=0&end=100", { headers: { Accept: "application/json" } });
      if (resp.status === 401) {
  ...
  async function saveReset() {
    saving = true; modalError = "";
    try {
      const resp = await fetch("/data/php/admin_user_reset_password.php", {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
...
<CMSHome>
  ...
  <table class="admin-table">
    <thead><tr><th>Email</th><th>Created</th><th>Updated</th><th>Actions</th></tr></thead>
    <tbody>
      {#each rows as r}
        <tr>
          <td>{r.email}</td>
          ...
          <td>{r.updated_at || ''}</td>
          <td>
            <button on:click={() => openReset(r)}>Reset password</button>
  ...
  >
    <div class="fields">
      <div class="field">
        ...
        <input id="user-email" type="email" bind:value={createEmail} required />
      </div>
      <div class="field">
        <label for="user-pass">Password</label>
        <input id="user-pass" type="password" bind:value={createPassword} required />
      </div>
    </div>
    {#if modalError}<p class="admin-error">{modalError}</p>{/if}
    <div class="actions">
      <button class="primary" on:click={saveCreate} disabled={!canCreate}>{saving ? 'Saving…' : 'Save'}</button>
      <button on:click={() => (newOpen = false)} disabled={saving}>Cancel</button>
    </div>
  </AdminModal>
  ...
  >
    <div class="fields">
      <div class="field">
        ...
        <input id="reset-pass" type="password" bind:value={resetPassword} required />
      </div>
    </div>
    {#if modalError}<p class="admin-error">{modalError}</p>{/if}
    <div class="actions">
      <button class="primary" on:click={saveReset} disabled={!canReset}>{saving ? 'Saving…' : 'Save'}</button>
      <button on:click={() => (resetOpen = false)} disabled={saving}>Cancel</button>
    </div>
  </AdminModal>
</CMSHome>
...
<style>
  ...
  input, label { box-sizing: border-box; max-width: 100%; }
</style>
