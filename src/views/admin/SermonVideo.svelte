...
import SermonModal from "./SermonModal.svelte";
	...
	let loading = false;
let modalOpen = false;
let modalInitial = { id: 0, date, title: "", src: "" };
...
function newSermon() {
	modalInitial = { id: 0, date: new Date().toISOString().slice(0,10), title: "", src: "" };
	modalOpen = true;
}

function editRow(row) {
	modalInitial = { id: row.id, date: row.date, title: row.title, src: row.src };
	modalOpen = true;
}
	...
	<SermonModal bind:open={modalOpen} initial={modalInitial} on:saved={loadList} on:close={() => (modalOpen = false)} />
			headers: {
				"Content-Type": "application/json",
				Accept: "application/json",
			},
