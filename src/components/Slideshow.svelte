    ...
    {:then images}
        ...
        {#if showCaptions}
            <div transition:fade class="controls">
                ...
                {:else}
                    <img on:click={()=>{paused = true; clearTimer()}} src="/icons/pause.svg" alt="Pause slideshow">
                {/if}
                <img on:click={()=>{clearTimer(); showImg = false}} src="/icons/next.svg" alt="Next slide">
                <div class="timer-container">
                    <div class="timer-fill" style="max-width: {(timeLeft * 100)/slideShowTime}%"></div>
                </div>
            </div>
        {/if}
    {/await}
...
<style lang='scss'>
    ...
    .caption {
        ...
        background: linear-gradient(rgba(0,0,0,0) 0%, rgba(0,0,0,.6) 30%, rgba(0,0,0,.8) 100%);
        color: white;
